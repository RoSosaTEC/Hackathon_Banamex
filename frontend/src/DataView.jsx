...
export default function DataView() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (selectedBranch) {
        params.append("id_branch", selectedBranch);
      }
      ...
      const url = `http://localhost:8000/filter-feedback?${params.toString()}`;
    ...
    if (scrollHeight - scrollTop <= clientHeight + 100) {
      ...
      if (visibleCount < filteredRows.length) {
        ...
        setVisibleCount((prev) =>
          Math.min(prev + 50, filteredRows.length)
        );
    ...
    <div className="flex-1 flex flex-col min-h-0 w-full relative">
      ...
      <div className="flex px-6 mb-4 gap-3 max-w-4xl items-center">
        ...
        <div className="flex items-center gap-1 border-2 border-brand-line px-2 py-1 bg-black text-xs font-light">
          ...
          <span className="opacity-40 text-[10px] uppercase mr-1">
            Desde:
          </span>
          ...
          <span className="opacity-40 text-[10px] uppercase mx-1">
            Hasta:
          </span>
      ...
      <div className="flex justify-between items-end mb-2 px-6">
        ...
        <p className="text-2xl font-light">
          ...
          Registros API:
          <span className="font-normal border-b border-brand-line/50 pb-1 ml-2">
            {filteredRows.length}
          </span>
      ...
      <div className="flex-1 border-t-2 border-brand-line flex flex-col min-h-0 bg-black w-full">
        ...
        <div className="grid grid-cols-8 w-full pt-4 px-4 border-b border-white/20 bg-black">

          {csvHeaders.map((headerName, index) => (

            <div
              key={index}
              className="pb-3 relative pr-4"
            >

              <h3 className="text-sm font-medium tracking-wide text-ellipsis overflow-hidden whitespace-nowrap opacity-90">
          ...
          {filteredRows.length === 0 ? (
            ...
            filteredRows
              ...
              .map((row, rowIndex) => (
                ...
                <div
                  key={rowIndex}
                  className="grid grid-cols-8 px-4 border-b border-white/5 hover:bg-white/5 transition-colors"
                >
