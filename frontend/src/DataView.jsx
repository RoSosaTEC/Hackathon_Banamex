import React, { useState, useEffect } from 'react';

export default function DataView() {

  const [csvHeaders, setCsvHeaders] = useState([]);
  const [csvRows, setCsvRows] = useState([]);
  const [visibleCount, setVisibleCount] = useState(50);
  const [loading, setLoading] = useState(false);

  const [searchTerm, setSearchTerm] = useState('');

  const [selectedBranch, setSelectedBranch] = useState('');

  const [uniqueBranches, setUniqueBranches] = useState([]);

  const [startDate, setStartDate] = useState('');

  const [endDate, setEndDate] = useState('');

  // -------------------------
  // FETCH DATA FROM FASTAPI
  // -------------------------

  const fetchData = async () => {
    try {
      setLoading(true);

      const params = new URLSearchParams();
      if (selectedBranch) params.append('id_branch', selectedBranch);
      if (startDate)      params.append('start_date', startDate);
      if (endDate)        params.append('end_date', endDate);

      const response = await fetch(`http://localhost:8000/filter-feedback?${params.toString()}`);
      const result = await response.json();
      const data = result.data || [];

      setCsvRows(data);

      if (data.length > 0) {
        setCsvHeaders(Object.keys(data[0]).filter(k => k !== 'cluster_name'));
        const branches = [...new Set(data.map(r => r.Id_branch).filter(Boolean))];
        setUniqueBranches(branches.sort());
      }
    } catch (error) {
      console.error('API ERROR:', error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { fetchData(); }, []);

  useEffect(() => {
    fetchData();
    setVisibleCount(50);
  }, [selectedBranch, startDate, endDate]);

  // -------------------------
  // INFINITE SCROLL
  // -------------------------

  const handleScrollGeneral = (e) => {
    const { scrollTop, scrollHeight, clientHeight } = e.currentTarget;
    if (scrollHeight - scrollTop <= clientHeight + 100) {
      if (visibleCount < filteredRows.length) {
        setVisibleCount(prev => Math.min(prev + 50, filteredRows.length));
      }
    }
  };

  // -------------------------
  // LOCAL TEXT SEARCH
  // -------------------------

  const filteredRows = csvRows.filter((row) => {
    if (!searchTerm) return true;

    const term = searchTerm.toLowerCase();
    return csvHeaders.some(h => String(row[h] || '').toLowerCase().includes(term));
  });

  // -------------------------
  // LOADING
  // -------------------------

  if (loading) {

    return (
      <div className="text-center py-20 text-xl font-light opacity-50 animate-pulse">
        Cargando datos desde FastAPI...
      </div>
    );

  }

  // -------------------------
  // RENDER
  // -------------------------

  return (

    <div className="flex-1 flex flex-col min-h-0 w-full relative">

      {/* BARRA DE FILTROS */}
      <div className="flex px-6 mb-4 gap-3 items-center bg-surface-raised py-4 border-b border-white/10">
        <input
          type="text"
          placeholder="Buscar texto..."
          value={searchTerm}
          onChange={(e) => { setSearchTerm(e.target.value); setVisibleCount(50); }}
          className="flex-1 min-w-[150px] bg-surface-elevated rounded-lg px-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-brand-line/50 placeholder-white/30 font-light transition-all"
        />

        {/* BRANCH SELECT */}

        <select
          value={selectedBranch}
          onChange={(e) => setSelectedBranch(e.target.value)}
          className="bg-surface-elevated rounded-lg px-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-brand-line/50 font-light cursor-pointer min-w-[160px] transition-all"
        >
          <option value="">Todas las sucursales</option>
          {uniqueBranches.map((branch, i) => (
            <option key={i} value={branch}>{branch}</option>
          ))}
        </select>

        <div className="flex items-center gap-2 bg-surface-elevated rounded-lg px-4 py-2 text-sm font-light">
          <span className="text-white/40 text-xs uppercase tracking-wide">Desde</span>
          <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-transparent text-white outline-none cursor-pointer text-sm" />
          <span className="text-white/20 mx-1">—</span>
          <span className="text-white/40 text-xs uppercase tracking-wide">Hasta</span>
          <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="bg-transparent text-white outline-none cursor-pointer text-sm" />
        </div>

      </div>

      {/* TOP INFO */}

      <div className="flex justify-between items-end mb-2 px-6">

        <p className="text-xs font-mono opacity-40">
          Mostrando {Math.min(visibleCount, filteredRows.length)} de {filteredRows.length} filas
        </p>

        <p className="text-2xl font-light">
          Registros: <span className="font-bold text-brand-red border-b-2 border-brand-red pb-1">{filteredRows.length}</span>
        </p>

      </div>

      {/* TABLA */}
      <div className="flex-1 border-t-2 border-brand-line flex flex-col min-h-0 bg-surface-base w-full">

        {/* HEADERS */}
        <div className="grid grid-cols-8 w-full pt-4 px-4 border-b border-brand-line/30 bg-surface-elevated">
          {csvHeaders.map((headerName, index) => (
            <div key={index} className="pb-3 relative px-3">
              <h3 className="text-sm font-medium tracking-wide text-ellipsis overflow-hidden whitespace-nowrap text-brand-blue">
                {headerName}
              </h3>
              {index !== csvHeaders.length - 1 && (
                <div className="absolute top-0 right-0 h-6 w-[2px] bg-brand-line opacity-40" />
              )}
            </div>

          ))}

        </div>

        {/* FILAS */}
        <div onScroll={handleScrollGeneral} className="flex-1 overflow-y-auto w-full scrollbar-none pb-8">
          {filteredRows.length === 0 ? (
            <div className="text-center py-20 opacity-40 font-light tracking-wide">
              Ningún registro coincide con los filtros.
            </div>
          ) : (
            filteredRows.slice(0, visibleCount).map((row, rowIndex) => (
              <div
                key={rowIndex}
                className={`grid grid-cols-8 px-4 border-b border-white/5 hover:bg-brand-blue/10 transition-colors ${rowIndex % 2 === 1 ? 'bg-surface-raised' : ''}`}
              >
                {csvHeaders.map((headerName, colIndex) => (
                  <div key={colIndex} className="py-3 px-3 relative min-h-[44px] flex items-center min-w-0">
                    <p className="text-xs font-light break-words leading-relaxed opacity-75 w-full">
                      {String(row[headerName] || '---')}
                    </p>
                    {colIndex !== csvHeaders.length - 1 && (
                      <div className="absolute top-0 right-0 h-full w-[2px] bg-brand-line opacity-20" />
                    )}
                  </div>
                ))}
              </div>
            ))
          )}

        </div>

      </div>

    </div>

  );
}
